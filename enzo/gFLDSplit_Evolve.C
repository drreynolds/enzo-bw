...
//   if (debug)  printf("Entering gFLDSplit::Evolve routine\n");
  ...
  if (Nchem > 0) 
    if (nHI == NULL) {
      fprintf(stderr,"gFLDSplit Evolve: cannot access HI density!\n");
      return FAIL;
    }
  if (Nchem > 1) {
    if (nHeI == NULL) {
      fprintf(stderr,"gFLDSplit Evolve: cannot access HeI density!\n");
      return FAIL;
    }
    if (nHeII == NULL) {
      fprintf(stderr,"gFLDSplit Evolve: cannot access HeII density!\n");
      return FAIL;
    }
  }
  ...
  if (StarMakerEmissivityField > 0) {
    ...
    float *EmissivitySource = ThisGrid->GridData->AccessEmissivityField0();
    ...
    // set flag that emissivity is filled in
    eta_set = 1;
  }
#endif
  ...
  if (this->ChemBounds(ThisGrid) != SUCCESS) {
      fprintf(stderr,"gFLDSplit Evolve: chemistry bound enforcement error\n");
      return FAIL;
  }
  ...
  if (GetUnits(&DenUnits, &LenUnits, &TempUnits, &TimeUnits, 
    ...
    fprintf(stderr,"gFLDSplit Evolve: Error in GetUnits.\n");
    return FAIL;
  }
  if (RadiationGetUnits(&RadUnits, tnew) != SUCCESS) {
    fprintf(stderr,"gFLDSplit Evolve: Error in RadiationGetUnits.\n");
    return FAIL;
  }
  ...
  for (radstep=0; radstep<=maxsubcycles*100; radstep++) {
	...
	if (this->ChemStep(ThisGrid, thisdt, tchem) != SUCCESS) {
	  fprintf(stderr,"gFLDSplit Evolve: Error in ChemStep routine\n");
	  return FAIL;
	}
    ...
    if (U0->exchange() != SUCCESS) {
      fprintf(stderr,"gFLDSplit Evolve: vector exchange error\n");
      return FAIL;
    }
...
int gFLDSplit::ChemStep(HierarchyEntry *ThisGrid, float thisdt, float tcur)
{
  ...
  if (GetUnits(&DenUnits0, &LenUnits0, &TempUnits, &TimeUnits, 
      ...
      fprintf(stderr,"gFLDSplit_ChemStep: Error in GetUnits.\n");
      return FAIL;
  }
  if (RadiationGetUnits(&RadUnits, tcur-thisdt) != SUCCESS) {
      fprintf(stderr,"gFLDSplit_ChemStep: Error in RadiationGetUnits.\n");
      return FAIL;
  }
  float mp = 1.67262171e-24;   // Mass of a proton [g]
  ...
  if (ComovingCoordinates) 
    if (CosmologyComputeExpansionFactor(tcur-thisdt, &a0, &adot0) != SUCCESS) {
      fprintf(stderr,"gFLDSplit_ChemStep: Error in CosmologyComputeExpansionFactor.\n");
      return FAIL;
    }
  ...
  rho = ThisGrid->GridData->AccessDensity();
  if (rho == NULL) {
    fprintf(stderr,"gFLDSplit_Chemstep: could not obtain density\n");
    return FAIL;
  }

  //   fill in the gas energy and chemistry source terms
  if (this->GasEnergySource(ecsrc, &tcur) != SUCCESS) {
    fprintf(stderr,"gFLDSplit_Chemstep: Error in GasEnergySource routine\n");
    return FAIL;
  }
  if (this->ChemistrySource(HIsrc, HeIsrc, HeIIsrc, &tcur) != SUCCESS) {
    fprintf(stderr,"gFLDSplit_Chemstep: Error in ChemistrySource routine\n");
    return FAIL;
  }
  
  //   solve local chemistry/gas energy systems
  if (this->AnalyticChemistry(U0, sol, extsrc, thisdt) != SUCCESS) {
    fprintf(stderr,"gFLDSplit_Chemstep: Error in AnalyticChemistry routine\n");
    return FAIL;
  }
...
int gFLDSplit::ChemBounds(HierarchyEntry *ThisGrid)
{
  ...
  rho = ThisGrid->GridData->AccessDensity();
  if (rho == NULL) {
    fprintf(stderr,"gFLDSplit_ChemBounds: could not obtain density\n");
    return FAIL;
  }
  nHI    = ThisGrid->GridData->AccessHIDensity();
  nHII   = ThisGrid->GridData->AccessHIIDensity();
  ...
  if (Nchem > 0) 
    if (nHI == NULL) {
      fprintf(stderr,"gFLDSplit_ChemBounds error: cannot access HI density!\n");
      return FAIL;
    }
  if (Nchem > 1) {
    if (nHeI == NULL) {
      fprintf(stderr,"gFLDSplit_ChemBounds error: cannot access HeI density!\n");
      return FAIL;
    }
    if (nHeII == NULL) {
      fprintf(stderr,"gFLDSplit_ChemBounds error: cannot access HeII density!\n");
      return FAIL;
    }
  }
...
int gFLDSplit::RadStep(HierarchyEntry *ThisGrid, int eta_set)
{
  ...
  if (GetUnits(&DenUnits0, &LenUnits0, &TempUnits, &TimeUnits, 
    ...
    fprintf(stderr,"gFLDSplit_RadStep: Error in GetUnits.\n");
    return FAIL;
  }
  if (RadiationGetUnits(&RadUnits, told) != SUCCESS) {
    fprintf(stderr,"gFLDSplit_RadStep: Error in RadiationGetUnits.\n");
    return FAIL;
  }
  float mp = 1.67262171e-24;   // Mass of a proton [g]
  ErUnits0 = RadUnits*ErScale;
  ...
  if (ComovingCoordinates) 
    if (CosmologyComputeExpansionFactor(told, &a0, &adot0) != SUCCESS) {
      fprintf(stderr,"gFLDSplit_RadStep: Error in CosmologyComputeExpansionFactor.\n");
      return FAIL;
    }
  adot0 /= TimeUnits;  // rescale to physical units
  ...
  if (eta_set == 0) {
    if (this->RadiationSource(RadSrc, &tnew) != SUCCESS) {
      fprintf(stderr,"gFLDSplit_RadStep: Error in RadiationSource routine\n");
      return FAIL;
    }
  }
  ...
  if (this->EnforceBoundary(U0) != SUCCESS) {
    fprintf(stderr,"gFLDSplit_RadStep: EnforceBoundary failure!!\n");
    return FAIL;
  }
  ...
  if (this->Opacity(OpacityE, &tnew, sol) != SUCCESS) {
    fprintf(stderr,"gFLDSplit_RadStep: Error in Opacity routine\n");
    return FAIL;
  }
    
  //   compute the gas 'Temperature' at old and new time steps
  if (this->ComputeTemperature(Temperature0, U0) != SUCCESS) {
    fprintf(stderr,"gFLDSplit_RadStep: Error in ComputeTemperature routine\n");
    return FAIL;
  }
  ...
  if (this->SetupSystem(matentries, rhsentries, &rhsnorm, RadiationEnergy, Eg_new, 
    ...
    fprintf(stderr,"gFLDSplit_RadStep: Error in SetupSystem routine\n");
    return FAIL;
  }
  if ((rhsnorm <= 0.0) || (rhsnorm != rhsnorm))  rhsnorm = 1.0;
  //  if ((rhsnorm <= 0.0) || isnan(rhsnorm))  rhsnorm = 1.0;
...
//     if ((finalresid*rhsnorm > sol_tolerance) || isnan(finalresid)) {
	...
	this->Dump(sol);
	fprintf(stderr, "Error in gFLDSplit_RadStep\n");
	return FAIL;
#ifndef FAIL_ON_NAN
      }
#endif
