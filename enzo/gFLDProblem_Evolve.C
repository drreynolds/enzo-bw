...
//   if (debug)  printf("Entering gFLDProblem::Evolve routine\n");
  ...
  if (vx == NULL) {
    fprintf(stderr,"gFLDProblem Evolve: could not obtain velocity1\n");
    return FAIL;
  }
  vy = ThisGrid->GridData->AccessVelocity2();
  if (vy == NULL) {
    fprintf(stderr,"gFLDProblem Evolve: could not obtain velocity2\n");
    return FAIL;
  }
  vz = ThisGrid->GridData->AccessVelocity3();
  if (vz == NULL) {
    fprintf(stderr,"gFLDProblem Evolve: could not obtain velocity3\n");
    return FAIL;
  }
  rho = ThisGrid->GridData->AccessDensity();
  if (rho == NULL) {
    fprintf(stderr,"gFLDProblem Evolve: could not obtain density\n");
    return FAIL;
  }
  if (DualEnergyFormalism) {
    eh = ThisGrid->GridData->AccessGasEnergy();
    if (eh == NULL) {
      fprintf(stderr,"gFLDProblem Evolve: could not obtain fluid energy\n");
      return FAIL;
    }
  }
  else {
  ...
  if (RadiationEnergy == NULL) {
    fprintf(stderr,"gFLDProblem Evolve: could not obtain Radiation energy\n");
    return FAIL;
  }
  ...
  if (StarMakerEmissivityField > 0) {
    ...
    if (EmissivitySource == NULL) {
      fprintf(stderr,"gFLDProblem Evolve: could not access emissivity field\n");
      return FAIL;
    }
    // access radiation source array 
    float *RadSrc = extsrc->GetData(0);
    if (RadSrc == NULL) {
      fprintf(stderr,"gFLDProblem Evolve: could not access Radiaton source\n");
      return FAIL;
    }
    // copy data
    for (i=0; i<ArrDims[0]*ArrDims[1]*ArrDims[2]; i++)
  ...
  if (U0->exchange_start() == FAIL) {
    fprintf(stderr,"gFLDProblem Evolve: vector exchange_start error\n");
    return FAIL;
  }
  ...
  if ((semi_implicit == 0) && (FRes > newt_tol)) {
    fprintf(stderr,"ERROR: non-convergent Newton method!\n");
    this->Dump(sol);
    return FAIL;
  }
