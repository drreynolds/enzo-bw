...
if (r_err_norm < tol):
    ...
    # plot I-front radii
    figure()
    plot(rdata[0],rdata[1]/rs0,'b-',rdata[0],r_sol/rs0,'r-')
    xlabel('$t/t_{rec}$')
    ylabel('$r_I/r_S$')
    ...
    grid()
    savefig('radii_vs_time' + pictype)

    # plot I-front radiius error
    figure()
    plot(rdata[0],(rdata[1]-r_sol)/rs0,'b-')
    xlabel('$t/t_{rec}$')
    ylabel('$r_I/r_S$ error')
    title('Error in HII Region Propagation')
    grid()
    savefig('radius_error_vs_time' + pictype)
